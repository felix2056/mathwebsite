...
function generate() {
    ...
    $.ajax({
        type: 'GET',
        url: "scripts/money-counting/generate.php",
        data: input,
...
function schedule() {
    Swal.mixin({
        ...
        if (result.value) {
            ...
            $.ajax({
                type: 'POST',
                url: "scripts/money-counting/schedule.php",
                dataType: "json",
...
function populateQuestions(result, mixed = false) {
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        html = html + `<li>`;
        question = question.split("moneyChoice").join(`mC${i}`);
...
function check_mC(q) {
    ...
    $.ajax({
        type: 'GET',
        url: "scripts/money-counting/check.php",
        data: {
            'ans': ans,
            'cans': cans
        },
