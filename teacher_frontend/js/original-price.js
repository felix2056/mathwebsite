...
function generate() {
    ...
    $.ajax({
        type: 'GET',
        url: "scripts/original-price/generate.php",
        data: input,
...
function schedule() {
    Swal.mixin({
        ...
        if (result.value) {
            ...
            $.ajax({
                type: 'POST',
                url: "scripts/original-price/schedule.php",
                dataType: "json",
...
function populateQuestions(result, mixed = false) {
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        html = html.replace("<q1>", `<input type="number" class="form-control" id="q_${i}1" ans='${answer[0]}'' >
            ...
            Check
         </b-button><div id="status_${i}1"  style="display: none;">${solution[0]}</div>`);
        html = html.replace("<q2>", `<input type="text" class="form-control" id="q_${i}2" ans='${answer[1]}' '>
            ...
            Check
         </b-button><div id="status_${i}2"  style="display: none;">${solution[1]}</div>`);
        html = html.replace("<q3>", `<input type="text" class="form-control" id="q_${i}3" ans='${answer[2]}''>
            ...
            Check
         </b-button><div id="status_${i}3"  style="display: none;>${solution[2]}</div>`);
         html = html.replace("<q4>", `<input type="text" class="form-control" id="q_${i}4" ans='${answer[3]}''>
            ...
            Check
         </b-button><div id="status_${i}4" style="display: none;>${solution[3]}</div>`);
         html = html.replace("<q5>", `<input type="text" class="form-control" id="q_${i}5" ans='${answer[4]}''>
            ...
            Check
         </b-button><div id="status_${i}5" style="display: none;>${solution[4]}</div>
         </li>`);
    }
...
function check(q) {
    ...
    $.ajax({
        type: 'GET',
        url: "scripts/original-price/check.php",
        data: {
            'ans': ans,
            'cans': cans
        },
